...
-- Better modulo
function bmod(i, N) return (i % N + N) % N end
...
function escape_single_bash(s) return escape_and_wrap(s, "'", "'\\''") end

-- Returns (a .. b) if b is not empty or nil
function joined_or_nil(a, b) return not isempty(b) and (a .. b) or nil end
...
function create_thumbnail_mpv(file_path, timestamp, size, output_path, options)
  ...
  local ytdl_disabled = not options.enable_ytdl
    and (
      mp.get_property_native("ytdl") == false
  ...
  if #header_fields > 0 then
    ...
    header_fields_arg = "--http-header-fields="
      .. table.concat(header_fields, ",")
  end
...
function check_output(ret, output_path, is_mpv)
  ...
  if ret.killed_by_us then
    ...
    if ret.error or ret.status ~= 0 then
      ...
      if is_mpv then msg.error("Debug log:", log_path) end
  ...
  if is_mpv and not thumbnailer_options.mpv_keep_logs then
    ...
    if success and file_exists(log_path) then os.remove(log_path) end
  end
...
function do_worker_job(state_json_string, frames_json_string)
  ...
  local generate_thumbnail_for_index = function(thumbnail_index)
    ...
    local timestamp = math.min(
      file_duration - 0.25,
      (thumb_idx + 0.5) * thumb_state.thumbnail_delta
    )

    mp.commandv(
      "script-message",
      "mpv_thumbnail_script-progress",
      tostring(thumbnail_index)
    )

    -- The expected size (raw BGRA image)
    local thumbnail_raw_size = (
    ...
    if thumbnail_file == nil then
      ...
      if existing_thumbnail_filesize ~= thumbnail_raw_size then
        ...
        msg.warn(
          "Thumbnail",
          thumb_idx,
          "did not match expected size, regenerating"
        )
        need_thumbnail_generation = true
      end
