...
function log(str)
  local logpath =
  utils.join_path(o.target_dir:gsub("~", get_homedir()), "mpv_slicing.log")
  f = io.open(logpath, "a")
...
function get_outname(shift, endpos)
  local nme = mp.get_property("filename")
...
function cut(shift, endpos)
  ...
  local inpath =
  escape(utils.join_path(utils.getcwd(), mp.get_property("stream-path")))
  local outpath = escape(
    utils.join_path(
